...
🆔 ID: `{user.id}`
📊 Plan: **{user_data['plan']}**
🎬 Calidad: **{QUALITIES.get(user_data['quality'], 'Media')}**
📦 Total compresiones: **{user_data['total_compressions']}**
💾 Total procesado: **{user_data['total_size'] / (1024**3):.2f} GB**
📅 Miembro desde: **{user_data['joined_date'].strftime('%d/%m/%Y')}**
"""
...
📌 **Título:** {info['title'][:100]}
⏱️ **Duración:** {info['duration'] // 60}:{info['duration'] % 60:02d}
...
"""
    ...
    await msg.edit_text(
        info_text,
        reply_markup=youtube_format_menu(info['formats']),
        parse_mode=ParseMode.MARKDOWN
    )
...
async def handle_video(client, message):
    ...
    msg = await message.reply_text(
        ...
        f"🎬 Calidad: {QUALITIES.get(user_data['quality'], 'Media')}",
        parse_mode=ParseMode.MARKDOWN
    )
    ...
    if result['success']:
        ...
        await msg.edit_text(
            ...
            f"💾 Nuevo tamaño: {result['compressed_size'] / (1024**2):.1f} MB",
            parse_mode=ParseMode.MARKDOWN
        )
        ...
        await message.reply_video(
            ...
            caption=f"✅ **Comprimido con CompresUltra**\n"
                   f"📊 Reducción: {result['compression_ratio']}%\n"
                   f"🎬 Calidad: {QUALITIES.get(user_data['quality'], 'Media')}"
        )
...
🎯 Plan: **{user_data['plan']}**
⏳ Expira: **{user_data['plan_expiry'].strftime('%d/%m/%Y') if user_data['plan_expiry'] else 'Nunca'}**
...
"""
        ...
        await callback_query.answer(f"✅ Calidad cambiada a {QUALITIES[quality]}")
        await callback_query.message.edit_text(
            f"✅ **Calidad actualizada:** {QUALITIES[quality]}",
            parse_mode=ParseMode.MARKDOWN
        )
...
async def report_command(client, message):
    if len(message.command) < 2:
        await message.reply_text("❌ **Uso:** `/reporte [descripción]`")
        return
